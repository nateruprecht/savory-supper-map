...
import { MapPin, Star, Plus, MessageSquare, ArrowLeft } from 'lucide-react';
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center mb-4">
            <Button 
              variant="ghost" 
              size="sm" 
              className="flex items-center text-supper-brown"
              onClick={() => navigate('/discover')}
            >
              <ArrowLeft className="mr-1 h-4 w-4" />
              Back to Discover
            </Button>
            <h1 className="text-xl font-semibold ml-2 text-supper-brown">Interactive Map</h1>
          </div>
        </div>
        
        <div className="md:grid md:grid-cols-5 md:gap-4 md:p-4 h-full">
          <div className="md:col-span-5 h-[50vh] md:h-[calc(100vh-12rem)] relative">
            <MapView clubs={clubs} onClubSelect={handleClubSelect} />
          </div>
          
          <div className="flex flex-row md:col-span-5 gap-2 py-4 px-4 justify-center">
            <Button variant="outline" className="flex-1 max-w-xs" onClick={() => setAddVisitOpen(true)}>
              <Plus className="mr-2 h-4 w-4" /> Add a Visit
            </Button>
            <Button variant="outline" className="flex-1 max-w-xs" onClick={() => setReviewFormOpen(true)}>
